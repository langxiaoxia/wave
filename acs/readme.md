...
AWS_UPLOAD: 0:不上传，1:上传但不输出日志，2:上传并且输出日志
...
AWS_UPLOAD=0 ./s3-upload-aws4.sh 'test.ts' 'ipc1' 'ipc-video-bucket-626676147343-us-east-1' 'us-east-1'

重点代码解释：
# 0. Hash the file to be uploaded
payloadHash=$(my_openssl dgst -sha256 -hex < "${fileLocal}" 2>/dev/null | my_sed 's/^.* //')
作用是对fileLocal文件内容使用SHA256算法做digest，再将digest结果以hex格式输出。
openssl输出结果为：
