...
FallbackDesktopCapturerWrapper::FallbackDesktopCapturerWrapper(
    ...
    : main_capturer_(std::move(main_capturer)),
      ...
      enable_border_(enable_border), //+by xxlang@2021-11-08
      first_capture_(true), //+by xxlang@2021-11-08
      window_border_(DesktopCapturer::CreateWindowBorder()) //+by xxlang@2021-11-08
{
  RTC_LOG(LS_WARNING) << "FallbackDesktopCapturerWrapper " << (enable_border_ ? "with" : "without") << " border";
...
bool FallbackDesktopCapturerWrapper::SelectSource(SourceId id) {
  if (SelectSourceInternal(id)) {
      RTC_LOG(LS_WARNING) << "FallbackDesktopCapturerWrapper::SelectSource " << selected_screen_ << " => " << id;
